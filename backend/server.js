...
app.post("/auth/signup", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ user: { id: user.id, uuid: user.uuid, email: user.email }, token });
  } catch (error) {
    console.error(error);
...
app.post("/trips", authenticateToken, async (req, res) => {
  const { origin, destination, mode, purpose, companions, startTime, endTime } =
    req.body;

  if (!origin || !destination || !mode || !startTime || !endTime) {
